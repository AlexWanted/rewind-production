...
import { Edit2, Trash2, GripVertical, UploadCloud, Plus } from 'lucide-react';
import { Reorder } from 'motion/react';
import { PhotoData } from '@/lib/supabase';

interface PhotoListProps {
  items: PhotoData[];
  onEdit: (item: PhotoData) => void;
  onDelete: (id: string) => void;
  onReorder: (items: PhotoData[]) => void;
  onSaveOrder: (items: PhotoData[]) => void;
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-light capitalize">Photos</h2>
        ...
        {items.map(item => (
          <Reorder.Item key={item.id} value={item} className="bg-zinc-900 p-4 flex justify-between items-center rounded-sm cursor-grab active:cursor-grabbing">
            <div className="flex items-center gap-4">
              ...
              <img src={item.images?.[0] || ''} alt={item.alt} className="w-24 h-16 object-cover rounded-sm bg-zinc-800" />
              <div>
